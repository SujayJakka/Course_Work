...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Add the variable nodes and edges based on the bayes net provided on the assignment document
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1]
    edges = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]

    # Array to keep track of all possible positions Pacman, Ghost0, and Ghost1 could be
    positions = []

    # Find all possible positions and append them to the positions list
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            positions.append((x, y))

    # Set the domain of Pacman, Ghost0, and Ghost1 to this positions list
    variableDomainsDict[PAC] = positions
    variableDomainsDict[GHOST0] = positions
    variableDomainsDict[GHOST1] = positions

    # Empty list to keep track of all possible distances for the observations
    distances = []
    # Calculate max possible distance by subtracting 1 from each dimension as distance is not inclusive of both endpoints of the range indexed at 0
    max_distance = (X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE

    # Append all possible distances to the distances array
    for i in range(max_distance + 1):
        distances.append(i)

    # Set the domain of Observation0 and Observation1 to this distances list
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Find all the factors and get only the variable assignments in each factor that match the evidence dict
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        # Iterate through each elimination variable in order
        for elimination_variable in eliminationOrder:
            # Join on the elimination variable
            not_joined_factors, joined_factor = joinFactorsByVariable(factors, elimination_variable)
            # Set factors to the factors that have not been joined
            factors = not_joined_factors

            # If the joined factor has only one unconditioned variable just continue as all of the rows in the CPT will just be 1, and it will have no impact on future joins
            if len(joined_factor.unconditionedVariables()) == 1:
                continue

            # Marginalize the elimination variable in the joined factor
            result_factor = eliminate(joined_factor, elimination_variable)
            # Append the result factor to your current list of factors
            factors.append(result_factor)

        # Join all remaining factors and then normalize
        joined_factor = joinFactors(factors)
        normalized_factor = normalize(joined_factor)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Find sum of all the values of the keys in the dictionary
        total = self.total()
        # If there are no keys or if they sum to 0 do nothing
        if len(self) == 0 or total == 0:
            return
        # If they do not sum to 0, normalize by diving by the sum
        for key in self:
            self[key] /= total
    ...
    def sample(self):
        ...
        # Sample based on the value weights
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        # If the ghost is in jail, then there is a 0% chance that noisy distance is not None
        if ghostPosition == jailPosition:
            if noisyDistance is None:
                return 1
            else:
                return 0
        # If the ghost is not in jail, then there is a 0% chance that noisy distance is None
        elif ghostPosition != jailPosition:
            if noisyDistance is None:
                return 0

        # Find true distance using manhattan distance
        true_distance = manhattanDistance(pacmanPosition, ghostPosition)
        # Return the P(noisy distance | true distance)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        # Loop through each position, set the posterior probability( P(Ghost Position | All Observations) to equal the prior times likelihood
        # Likelihood here is P(New Observation | Ghost Position)
        # Prior is P(Ghost Position | Previous Observations)
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        # Create a new distribution for the next time step
        new_beliefs = DiscreteDistribution()

        # Loop through all the positions
        for old_position in self.allPositions:
            # Find the prob of the old position
            prob_old_position = self.beliefs[old_position]
            # Find the prob distribution of the next position given the old position
            new_pos_dict = self.getPositionDistribution(gameState, old_position)

            # Add the product of the prob of the old position and the prob of the new position given the old position to
            # each new position. Marginalizing across old positions to find the prob distribution of the next position.
            for new_position in new_pos_dict:
                new_beliefs[new_position] += (prob_old_position * new_pos_dict[new_position])
