...
def svm_loss(x, y):
    ...
    # Compute Loss
    correct_scores = x[np.arange(num_train), y].reshape(-1, 1)
    loss_matrix = x - correct_scores + 1
    loss_matrix = np.maximum(0, loss_matrix)
    loss_matrix[np.arange(num_train), y] = 0
    loss = np.sum(loss_matrix) / num_train

    # Compute dx
    boolean_loss_matrix = (loss_matrix > 0).astype(int)
    row_sum_vector = np.sum(boolean_loss_matrix, axis = 1)
    boolean_loss_matrix[np.arange(num_train), y] -= row_sum_vector
    dx = boolean_loss_matrix / num_train
