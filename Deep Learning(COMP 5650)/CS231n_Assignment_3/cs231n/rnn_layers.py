...
def rnn_step_backward(dnext_h, cache):
    ...
    d_affine = (1 - np.power(np.tanh(affine), 2)) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, H = np.shape(x)[0], np.shape(x)[1], np.shape(h0)[1]
    h = np.zeros((T, N, H))
    cache_arr = []

    for i in range(T):
        next_h, cache_item = rnn_step_forward(x[:, i, :], h0, Wx, Wh, b)
        h[i] = next_h
        cache_arr.append(cache_item)
        h0 = next_h
    
    h = np.transpose(h, (1, 0, 2))
    cache = (x, h0, Wx, Wh, b, cache_arr)
...
def rnn_backward(dh, cache):
    ...
    x, h0, Wx, Wh, b, cache_arr = cache
    N, T, D, H = np.shape(x)[0], np.shape(x)[1], np.shape(Wx)[0], np.shape(Wx)[1]
    ...
    for i in range(T - 1, -1, -1):
        dcurr_h += dh[:, i, :]
        dx_step, dprev_h, dWx_step, dWh_step, db_step = rnn_step_backward(dcurr_h, cache_arr[i])
