...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
            ...
            embedded_captions_in = W_embed[captions_in]
            ...
            dh = d_scores.dot(W_vocab.T)
            dx, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn)
            grads["W_vocab"] = h.reshape(-1, h.shape[-1]).T.dot(d_scores.reshape(-1, d_scores.shape[-1]))
            grads["b_vocab"] = np.sum(d_scores, axis=(0, 1))
            grads["Wx"] = dWx
            grads["Wh"] = dWh
            grads["b"] = db
            grads["W_proj"] = features.T.dot(dh0)
            grads["b_proj"] = np.sum(dh0, axis=0)
            grads["W_embed"] = np.zeros_like(W_embed)
            for i in range(dx.shape[0]):
                for t in range(dx.shape[1]):
                    grads["W_embed"][captions_in[i, t]] += dx[i, t]
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == "rnn":
            ...
            for t in range(max_length):
                ...
                if t == 0:
                  h, _ = rnn_step_forward(word_embed, h0, Wx, Wh, b)
                else:
