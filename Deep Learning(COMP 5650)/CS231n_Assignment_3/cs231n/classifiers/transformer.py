...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        visual_embeddings = self.visual_projection(features)

        visual_embeddings = visual_embeddings.unsqueeze(1)
        visual_embeddings = visual_embeddings.expand(N, T, -1)

        tgt_mask = torch.tril(torch.ones(T, T))

        decoder_output = self.transformer(caption_embeddings, visual_embeddings, tgt_mask=tgt_mask)
